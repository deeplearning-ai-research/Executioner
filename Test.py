...
with Executioner() as executioner:
    ...
    print(Y.to_nparray("y1"))
    print(Y[:,"y1"].to_nparray())
    print(Y[:,"y1"].to_list())
    print(Y["y1"])
...
# 
# with Executioner() as executioner:
#     executioner.onStart(Execute("python dtlz2_socket.py"))
#     executioner.onStart(Pause(1))
#     executioner.onStart(Connect(server=socket.gethostname(), port=9156))
